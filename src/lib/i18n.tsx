...
const dict: Record<Lang, Dict> = {
  en: {
    ...
    moversNoHistoryNote: "Historical rates are a fixed yearly snapshot table (1-5 years back), fetched once via api.frankfurter.dev (ECB + 50+ central banks) and refreshed monthly - not a live per-request fetch, so this never times out or flickers. It covers about 30 major currencies; destinations in currencies outside that set just won't appear here (but do appear in \"Cheapest right now\").",
    ...
    methodologyLink: "How our indices work",
  ...
  he: {
    ...
    moversNoHistoryNote: "הנתונים ההיסטוריים הם טבלת תמונות-מצב שנתיות קבועות (1-5 שנים אחורה), שנשלפות פעם אחת דרך api.frankfurter.dev (ה-ECB ו-50+ בנקים מרכזיים) ומתעדכנות פעם בחודש - לא שליפה חיה לכל בקשה, אז זה לעולם לא נתקע או מרצד. הכיסוי הוא כ-30 מטבעות מרכזיים; יעדים במטבעות מחוץ לרשימה הזו פשוט לא יופיעו כאן (אבל כן מופיעים ב\"זולים עכשיו\").",
    ...
    methodologyLink: "איך המדדים שלנו עובדים",
  ...
  es: {
    ...
    moversNoHistoryNote: "Los datos históricos son una tabla fija de instantáneas anuales (1-5 años atrás), obtenida una vez vía api.frankfurter.dev (BCE + 50+ bancos centrales) y actualizada mensualmente - no es una consulta en vivo por solicitud, así que nunca se cuelga ni parpadea. Cubre unas 30 monedas principales; los destinos en otras monedas simplemente no aparecerán aquí.",
    ...
    methodologyLink: "Cómo funcionan nuestros índices",
  ...
  fr: {
    ...
    moversYearsUnit: "ans",
    moversNoHistoryNote: "Les données historiques sont un tableau fixe d'instantanés annuels (1 à 5 ans en arrière), récupéré une fois via api.frankfurter.dev (BCE + 50+ banques centrales) et mis à jour mensuellement - ce n'est pas une requête en direct à chaque fois, donc cela ne se bloque ni ne clignote jamais. Couvre environ 30 devises majeures ; les destinations dans d'autres devises n'apparaîtront simplement pas ici.",
    ...
    methodologyLink: "Comment fonctionnent nos indices",
